...
export default function DistrictSecretariesApprovalPage() {
    ...
    const fetchSecretaries = async () => {
        try {
            setLoading(true);
            const response = await axios.get('http://localhost:5001/api/v1/district-secretaries', {
    ...
    const handleAction = async (id: string, status: 'APPROVED' | 'REJECTED') => {
        ...
        setActionLoading(id);
        try {
            await axios.put(`http://localhost:5001/api/v1/district-secretaries/${id}/status`,
                { status },
                { headers: { Authorization: `Bearer ${token}` } }
            );
            fetchSecretaries();
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-white">District Secretary Approvals</h1>
                    <p className="text-slate-400">Manage district secretary registrations</p>
                </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-1 text-sm text-slate-400">
                                    ...
                                    <div className="flex items-center gap-2 text-white font-medium mt-1">
                                        <MapPin className="w-4 h-4 text-blue-400" /> District: {sec.district.name}, {sec.state.name}
                                    </div>
